...
pub fn main() {
    ...
    // Keep a handle to the inner bus: main observes it non-blockingly below,
    // while the workers get the blocking CondvarBus wrapper.
    let circular = rust_messenger::message_bus::atomic_circular_bus::CircularBus::new(&config);
    let message_bus = rust_messenger::message_bus::condvar_bus::CondvarBus::new(circular.clone());
